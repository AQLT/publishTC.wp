...
Même si la moyenne mobile CLF permet de réduire le nombre d'oscillations indésirables et les révisions dans les estimations intermédiaires, c'est au coût d'un biais plus important dans l'approximation d'une tendance de degré 1 qui n'est pas mesuré par ces indicateurs.
...
```{r}
#| label: fig-ex-covid
#| fig.cap: "Lissage de la série de l'indice de production industrielle (IPI) CVS-CJO dans l'industrie manufacturière (CZ) autour de la périodex du COVID-19."
...
Soit $y_{t-h},\dots,y_{t}$ la série observée sur les $h$ dernières dates (lesquelles les moyennes mobiles asymétriques sont utilisées pour obtenir les estimations intermédiaires aux dates $t-h$ to $t$).
...
Ce graphique est réalisable à partir des fonctions `publishTC::rowthplot()` et `publishTC::gggrowthplot()`.
...
Les taux de croissance de la série désaisonnalisée peuvent être représentés en ligne en utilisant le paramètre `sa_bar_line = TRUE`.
...
(gggrowthplot(tc_h, xlim = c(2022, NA), lag = -4) +
...
9. Faut-il ajouter des informations générales ? Il s'agit pour chaque méthode les valeurs de l'I/C ratio, MCD et la longueur de la moyenne mobile symétrique seront rajouter et également des points atypiques modélisés pour le lissage par moyennes mobiles robustes.
