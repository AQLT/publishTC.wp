...
:::: {.abstract}
Il est commun de désaisonnaliser des séries économiques afin d'étudier la conjoncture et déterminer l'état du cycle dans lequel se trouve l'économie.
En supprimant la composante saisonnière, les évolutions des séries désaisonnalisées sont influencées à la fois par la tendance-cycle et de l'irrégulier.
Lorsque l'irrégulier est important, il est préférable de lisser la série pour supprimer l'irrégulier et analyser l'effet combiné de la tendance de long terme et des cycles de court terme appelé composante tendance-cycle.
...
Cet article détaille également les recommandations de présentation de cette composante et comment mettre facilement en production son estimation, en l'appliquant à une dizaine de publications de l'Insee (environ 80 séries).
...
By removing the seasonal component, variations in seasonally-adjusted series are influenced by both the trend-cycle and the irregularity.
When the irregular component is significant, it is preferable to smooth the series to remove the irreguls and analyze the combined effect of the long-term trend and short-term cycles, known as the trend-cycle component.
...
This article also describes the recommendations for the presentation of this component and how to easily put its estimation into production, by applying it to a dozen Insee publications (around 80 series).
...
# Qu'est-ce que la tendance-cycle, pourquoi l'estimer et la publier ?
...
Pour simplifier les notations ultérieures, $y_t$ désignera la série observée transformée et, puisque l'on s'intéresse à l'estimation de la tendance-cyce, on considérera la série désaisonnalisée ou sans saisonnalité ($S_t=0$) :
$$
...
Toutefois, lorsque l'irrégulier est important, sa présence peut perturber l'analyse dans certains cas d'usage (comme l'analyse de la direction sous-jacente de la série ou la détection de points de retournements) : l'analyse de la tendance-cycle parait alors plus appropriée car tous les effets saisonniers et irréguliers sont éliminés de la série originale.
...
Inversement, la tendance-cycle est, par construction, plus lisse et la direction sous-jacente est clairement lisible.
...
Comme notamment indiqué par @mclaren_zhang_2010, les trois principales raisons de calculer et publier la tendance-cycle, en plus de la série désaisonnalisée, sont :
...
Toutefois, les seuls instituts statistiques qui diffusent régulière cette composante (Statistique Canada et l'Australian Bureau of Statistics (ABS)), utilisent des méthodes basées sur des moyennes mobiles avec une méthodologie similaire à celle utilisée dans la méthode de désaisonnalisation X-13ARIMA-SEATS (qui utilise une succession de moyennes mobiles différentes ayant chacune des propriétés spécifiques).
...
Voir notamment @inseeDTM202401 pour une description plus complètes des méthodes récentes de construction de moyennes mobiles pour l'estimation de la tendance-cycle.
...
Pour l’extraction de la tendance-cycle, le moyenne symétrique la plus connue est celle de @henderson1916note : elle est notamment utilisé dans l'algorithme de désaisonnalisation X-13ARIMA-SEATS.
...
Dans X-13-ARIMA-SEATS, la longueur de la moyenne mobile (i.e. : le paramètre $h$ déterminant le nombre de points utilisés pour l'estimation de la tendance-cycle) est déterminée à partir d'un ratio $R$ appelé I/C ratio défini par :
$$
...
Le calcul de ce ratio nécessite de disposer d'une estimation de la tendance-cycle : en notant $s$ la fréquence de la série ($s=12$ pour une série mensuelle, $s=4$ pour une série trimestrielle...), une première estimation de la tendance-cycle est effectuée à partir d'une moyenne mobile d'ordre $h=s/2$
...
Cela permet de réduire les révisions et de réduire le délai dans la détection des points de retournement, sans changer les estimations finales de la tendance-cycle (toujours estimées via une moyenne mobile de Henderson).
...
Du fait de leur présence, l'application directe des méthodes peut donc conduire à des estimations biaisées alors que les méthodes de désaisonnalisation (comme la méthode X-13ARIMA) ont un module de correction des points atypiques.
...
Les *cascade linear filter* [CLF, @clf], correspondent à une approximation des NLDF où l'on n'a tronqué la moyenne mobile afin qu'elle soit de longueur 13 et en effectuant une procédure spécifique de normalisation des poids (de sorte à préserver sans biais les constantes).
...
Même si la moyenne mobile CLF permet de réduire le nombre d'ondulations indésirables et les révisions dans les estimations intermédiaires, c'est au coût d'un biais plus important dans l'approximation d'une tendance de degré 1 qui n'est pas mesuré par ces indicateurs.

Cette méthode d'estimation de la tendance-cycle peut être appliquée en utilisant la fonction `publishTC::clf_smoothing()`.
Par défaut, pour les moyennes mobiles asymétriques, la méthode «\ couper-et-normaliser\ » est utilisée : les moyennes mobiles ALF peuvent être utilisées en utilisant l'option `endpoints = "ALF"`.
...
L'objectif de cette section est de décrire les principales recommandations concernant la publication et la présentation de la tendance-cycle.
...
La principale recommandation est la tendance-cycle doit être présentée conjointement avec la série désaisonnalisée [@abs_time_series_guidelines; @martin2009issuestc; @mclaren_zhang_2010; @imf_qna_2017].
...
De nombreux auteurs s'accordent donc à recommander que la présentation des estimations de la tendance-cycle doit mettre en lumière le moindre degré de fiabilité de la tendance-cycle pour les dernières observations [ @huot1989note; @kenny1982local; @imf_qna_2017].
...
C'est ce qui est fait par défaut dans les `publishTC` avec les quatre dernières estimations (comme dans les publications de Statistique Canada) dans les fonctions `plot()` et `ggplot2::autoplot()`.
...
@martin2009issuestc recommande que le nombre de mois en pointillés soit déterminé par la statistique MCD (*month of cyclical dominance*) qui est le nombre de mois ou de trimestres nécessaires pour que la variation moyenne absolue de la tendance-cycle soit supérieure à celle de l'irrégulier.
...
La statistique MCD est le premier délai $d$ pour lequel tous les ratios $(R_k)_{k\geq d}$ sont inférieurs à 1 : il s'agit donc du nombre de mois pour que la composante tendance-cycle deviennent donc dominante par rapport à la composante irrégulière.
...
L'inconvénient de cette représentation est que cela peut laisser croire qu'il n'y a pas d'incertitude autour des estimations de la série brute ou désaisonnalisée.
...
Afin de mettre en avant les évolutions de la tendance-cycle, pour représenter les taux de croissance des séries @mclaren_zhang_2010 suggèrent de faire un graphique en bâton pour les évolutions de la série désaisonnalisée plutôt que lignes.
Cela permet d'éviter d'accorder trop d'importance aux évolutions de la série désaisonnalisée qui peuvent être fortement influencées par l'irrégulier et de donner une direction claire des mouvements cycliques en indiquant la position dans le cycle.
...
Un modèle simplifié de mise en production permet pour chaque publication de créer un rapport qui met automatiquement à jour les séries tous les mois.
...
Cette section décrit ce rapport est comment facilement ajouter de nouvelles publications ou de nouvelles séries.
...
series:
  ...
  ipi_c5:
    [...]
```
1. Nom de stockage des séries dans la base de données.
...
6. Dates associées aux points atypiques additifs dont l'effet est associé à la tendance-cycle (cas particulier non décrit dans cet article).
...
Par ailleurs, afin d'éviter des révisions sur les points atypiques retenus, ces derniers ne sont plus modifiés : lors d'une mise en production, il est recommandé d'analyser les nouveaux points afin de déterminer s'ils ne faut pas en rajouter.
...
Par ailleurs, puisque l'objet d'intérêt est principalement les observations récentes, il n'a pas paru nécessaire d'étudier le passé lointain.

De la même façon que dans les publications de Statistique Canada et ABS, la longueur de la moyenne mobile symétrique est fixée à 13 termes pour l'ensemble des publications.
...
3. Faut-il ajouter un bloc avec l'ensemble des graphiques pour chaque méthode ? Bloc ici non affiché.
...
10. Faut-il ajouter des informations générales ? Il s'agit pour chaque méthode les valeurs de l'I/C ratio, MCD et la longueur de la moyenne mobile symétrique seront rajouter et également des points atypiques modélisés pour le lissage par moyennes mobiles robustes.
...
Par rapport au mois de mai 2025, la révision totale de la tendance-cycle correspond donc pour chaque date à $\widehat TC_{t} (y_{t|2025.05}) - \widehat TC_{t} (y_{t|2025.04}).$
...
En somme, afin d'éclairer le débat public et réduire le risque que les utilisateurs tirent des conclusions inappropriées, il peut être utile de publier la composante tendance-cycle en même temps que les séries désaisonnalisées, notamment lorsque ces dernières sont bruitées.
...
Les seuls instituts publiant régulièrement la composante tendance cycle, Australian Bureau of Statistics (ABS) et Statistique Canada, utilisent des méthodes basées sur des moyennes mobiles.
...
Ce dernier permet également de reproduire le lissage avec les méthodes utilisées par ABS et Statistique Canada, ainsi que de facilement produire des visualisations.
Sa facilité d'utilisation permet de facilement mettre en place une chaîne de production de la composante tendance-cycle : un modèle simplifié a été mis en place dans cet article autour de séries de l'Insee permettant de facilement ajouter de nouvelles publications et de nouvelles séries.
...
Pour une mise en production officielle de la tendance-cycle il est toutefois recommandé d'analyser précisément les résultats en utilisant l'expertise économique que le producteur a sur les séries, permettant notamment de choisir la méthode représentant la réalité économique de la série.
