...
for (serie in names(all_data[[1]])) {
	...
	if (spec$plots$bymethods$enabled){
		...
		for (methods in names(last_est_s)) {
			...
			cat(knitr::knit_child(text = c(
				"```{r}",
				sprintf("#| label: bymethods-%s-%s", serie, studied_methods[methods]),
	...
	if (spec$plots$byplots$enabled){
		...
		for (plot in names(byplots)) {
			...
			cat(knitr::knit_child(text = c(
				"```{r}",
				sprintf("#| label: byplots-%s-%s", serie, studied_methods[methods]),
	...
	if (spec$plots$revision_history$enabled) {
		...
		for (methods in names(rev_data_s)) {
			...
			cat(knitr::knit_child(text = c(
				"```{r}",
				sprintf("#| label: revisions-%s-%s", serie, studied_methods[methods]),
