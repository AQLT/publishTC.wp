...
if (!is.null (spec$plots$revision_history$nobs)) 
    rev_data <- tail(rev_data, spec$plots$revision_history$nobs)
...
rev_data_fixed <- lapply(series, function(serie) {
    ...
    dates <- all_data[-n]
    if (!is.null (spec$plots$revision_history$nobs)) 
        dates <- tail(dates, spec$plots$revision_history$nobs - 1)
...
rev_tot <- lapply(series, function(serie) {
    ...
    rev_tot_by_method <- do.call(
        ...
        lapply(names(rev_by_method), function(method){
            rev_meth <- rev_to_table(rev_by_method[[method]], nobs = spec$plots$revision_history$nobs)
